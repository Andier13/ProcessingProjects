...
class particle{
  ...
  color col;
  boolean collided, pressed = false;
  ...
  void applyCollisionOffset(){
    if (collided){
      pos = PVector.add(ppos, colloff);
      colloff = new PVector(0, 0);
      collided = false;
    }
  }
  
  float getPotentialEnergy(box b){
    return m * g.mag() * (b.bottom - pos.y);
  }
  
  float getKineticEnergy(box b){
    float v = vel.mag();
    return m * v * v * 0.5;
  }
  
  void pressed(){
    if ((pos.x-mouseX)*(pos.x-mouseX)+(pos.y-mouseY)*(pos.y-mouseY)<=r*r)
